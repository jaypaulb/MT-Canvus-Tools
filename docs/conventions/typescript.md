...
**Decision:** `createAnyWithAsset` is NOT added to the TS SDK. The `createAny` method remains JSON-only for non-asset widget types (Note, Anchor, Browser, Connector, Table, VideoInput). For image/video/pdf uploads, callers MUST use the typed sub-resource upload methods: `widgets.images.upload`, `widgets.videos.upload`, `widgets.pdfs.upload`. `createAny` throws `UnsupportedOperationError` with an explicit redirect message when called with an asset widget type.
**Rationale:** Go's `CreateWidget(io.Reader)` is a shotgun-pattern helper that tries CreateImage → CreatePDF → CreateVideo sequentially with no content-type discrimination. The TS SDK already provides a strictly superior API: typed upload methods (`images.upload`, `videos.upload`, `pdfs.upload`) that know their endpoint, accept `Blob | Buffer`, and produce typed return values. Adding `createAnyWithAsset` would replicate Go's inferior pattern under a new name, duplicate functionality already cleanly available, and widen the generic surface unnecessarily. The transport already handles `FormData` (`buildUploadForm` in widgets.ts, used by all three typed upload sub-resources). TS consumers of any widget type are fully served; the asymmetry versus Go is intentional and beneficial — it keeps the typed surface paramount per the §4.3 design rationale.

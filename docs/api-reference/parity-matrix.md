...
- **Permissions subscribe in Python/Go** — TS exposes both canvas and folder permission subscribe. The streaming spec lists these as supported. **Verified 2026-05-19 against dev-mtcs.multitaction.com** (Phase 4d Round D Task D1): both `SubscribeCanvasPermissions`/`SubscribeFolderPermissions` (Go) and `canvases.subscribe_permissions` (Python) emit an initial snapshot followed by a change event whenever the corresponding `POST /canvases/{id}/permissions` or `POST /canvas-folders/{id}/permissions` is issued. Live tests: `go/sdk/canvus/permissions_subscribe_live_test.go` (build tag `live`) and `python/sdk/tests/test_permissions_live.py` (`pytest -m live`). The Python `FoldersResource` does not yet expose a `subscribe_permissions` helper — adding one is a follow-up parity item, not a verification gap.
